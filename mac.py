...
class Mac(object):
    def __init__(self, node):
        ...
        self.agent = agent.agent(self, len(parameters.action_space), parameters.learning_rate, parameters.reward_decay, parameters.e_greedy)
    ...
    def send(self, destination, payloadLength, id, begin_time):
        ...
        self.sensing = self.env.process(self.waitIdleAndSend(macPkt, begin_time))
        yield self.sensing
    ...
    def waitAck(self, macPkt, state, action, begin_time):
        # 重传
        try:
            ...
            if macPkt.retransmitiontimes > parameters.MAX_RETRANSMITION_TIME:
                if parameters.PRINT_LOGS:
                    ...
                    new_state = [state[0], -1]
                    if parameters.PRINT_LOGS:
                        print('Node %s begins to learn, reward is %f' % (self.name, parameters.REWARD_of_transmissionfailing))
                    self.agent.learn(str(state), action, parameters.REWARD_of_ackfailing, str(new_state), self.agent.action_choosing(str(new_state)))
            else:    
                ...
                if parameters.PRINT_LOGS:
                    print('Node %s begins to learn, reward is %f' % (self.name, parameters.REWARD_of_ackfailing))
                self.agent.learn(str(state), action, parameters.REWARD_of_ackfailing, str(new_state), self.agent.action_choosing(str(new_state)))
            ...
            # RL learning
            # TODO: deal with the case of the end of a packet
            new_state = [state[0], 0]
            reward = 100000/(self.env.now - begin_time)
            if parameters.PRINT_LOGS:
                print('Node %s begins to learn, reward is %f' % (self.name, reward))
            self.stats.logrewards(reward, self.env.now)
            self.agent.learn(str(state), action, reward, str(new_state), self.agent.action_choosing(str(new_state)))



    def waitIdleAndSend(self, macPkt, begin_time): 
        # 闲置等待并发送
        self.isSensing = True

        timeout = parameters.DIFS_DURATION
        # RL state
        state = [macPkt.destination, self.retransmissionCounter[macPkt.id]]
        # RL action choice
        action = self.agent.action_choosing(str(state))
        print(parameters.action_space[action])
        backoff = (parameters.SLOT_DURATION/2) * int(parameters.action_space[action])
        timeout += backoff
        # backoff = 0
        ...
        while True:
            try:
                ...
                self.pendingPackets[macPkt.id] = self.env.process(self.waitAck(macPkt, state, action, begin_time))
                self.isSensing = False
                return
            except simpy.Interrupt:
